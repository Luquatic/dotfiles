...
load_dotenv("/etc/tokens/open-weather.env")
...
def print_help():
    ...
    print(
        " - \x1b[32mgeocoding\x1b[0m <city[,state][,country]> : search for city to get its coordinates"
    )
    print(
        " - \x1b[32mcurrent\x1b[0m                            : print current weather information"
    )
    print(
        " - \x1b[32mforecast[-daily]\x1b[0m                   : print forecast for the next ~5 days"
    )
    print(
        " - \x1b[32mforecast-detail\x1b[0m                    : print detailed forecast in 3h intervals"
    )
    print(
        " - \x1b[32mwaybar\x1b[0m                             : get output for usage with waybar"
    )
...
def current_weather():
    ...
    print_entry(
        "temp",
        f"\x1b[33m{temperature} °C\x1b[90m, feels like \x1b[33m{temperature_felt} °C\x1b[0m",
    )
...
def get_daily_forecast_data() -> dict[dict]:
    ...
    for day in sorted(res):
        ...
        weather_count = {
            i: weather_descriptions.count(i) for i in set(weather_descriptions)
        }
        weather_count_max = max(weather_count.values())
        weather_average = tuple(
            filter(
                lambda x: weather_count[x] == weather_count_max, weather_count.keys()
            )
        )[0]
...
def daily_forecast():
    ...
    for day in sorted(daily_data):
        ...
        print_entry(
            "temp",
            f"\x1b[33m{data['max_temperature']} °C\x1b[0m / \x1b[33m{data['min_temperature']} °C\x1b[0m",
            indent=2,
        )
        print_entry(
            "humidity", f"\x1b[31m{data['humidity_average']} % RH\x1b[0m", indent=2
        )
        print_entry("wind", f"\x1b[35m{data['wind_average']} m/s\x1b[0m", indent=2)
        print_entry(
            "rain",
            f"\x1b[34m{data['rainfall_total_estimated']} mm\x1b[0m"
...
def waybar_current(data: dict) -> str:
    ...
    output += waybar_entry(
        "temp",
        f"{colorize(f'{temperature} °C', ORANGE)}{colorize(', feels like ', GRAY)}{colorize(f'{temperature_felt} °C', ORANGE)}",
    )
    output += waybar_entry("humidity", colorize(f"{humidity} % RH", RED))
    output += waybar_entry(
        "wind",
        f"{colorize(f'{wind_speed} m/s', PURPLE)} {colorize(f'({wind_direction})', GRAY)}",
    )
...
def waybar():
    ...
    tooltip = (
        colorize("current weather", BLUE)
        + "\n"
        + current
        + "\n"
        + colorize("forecast", BLUE)
        + "\n"
        + forecast
    )
